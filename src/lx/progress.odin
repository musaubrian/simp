...
progress :: proc(parent: ^Box, debug_label: string, w, h: f32, value: f32, size_mode := Size_Mode.Mixed, style := Style{}) {
    ...
    track := box(debug_label, w, h, size_mode = size_mode, style = { bg = _Progress_Track , round = style.round, padding = track_padding })
    fill_w := clamp(value, 0, 1)

    fill_bg := style.bg if style.bg != {} else _Progress_Fill
